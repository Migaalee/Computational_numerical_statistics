#Custom functions PDF, CDFs, etc-------------
#Uniform distribution discrete----------------

dunifdisc<-function(x, min=0, max=1) ifelse(x>=min & x<=max & round(x)==x, 1/(max-min), 0)
punifdisc<-function(q, min=0, max=1) ifelse(q<min, 0, ifelse(q>=max, 1, (floor(q)-min)/(max-min)))
qunifdisc<-function(p, min=0, max=1) floor(p*(max-min))
